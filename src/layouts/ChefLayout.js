// layouts/ChefLayout.js - CÓ NOTIFICATION BELL + NHẬN SỰ KIỆN TỪ CHEFDASHBOARD
...
const ChefLayout = () => {
    ...
    // ===== NHẬN NOTIFICATIONS TỪ CHEFDASHBOARD =====
    useEffect(() => {
        const handleChefNotification = (event) => {
            const { message, type, timestamp } = event.detail;
            const id = Date.now() + Math.random();

            console.log('🔔 ChefLayout nhận notification:', { message, type });
        ...
        return () => {
            window.removeEventListener('chef-notification', handleChefNotification);
        };
    }, []);
    ...
    const menuItems = [
        { label: "👨‍🍳 Khu bếp", path: "/chef/dashboard" },
        { label: "📅 Ca làm", path: "/chef/shift" },
    ];

    const isActive = (path) => {
        if (path === '/chef/dashboard') {
            return location.pathname === '/chef' || location.pathname === '/chef/dashboard';
        }
    ...
    return (
        <div style={{ minHeight: '100vh', background: '#f1f5f9' }}>
            {/* Header với menu + notification bell cùng hàng */}
            <div style={{
                background: '#1e293b',
                color: 'white',
                padding: '12px 24px',
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                    ...
                    <h2 style={{ margin: 0, fontSize: 20 }}>👨‍🍳 Bếp</h2>
