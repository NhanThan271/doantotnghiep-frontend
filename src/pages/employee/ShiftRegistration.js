    ...
    <Card className={`stat-card ${variant} fade-in-up`}>
        <Card.Body>
            <div className="d-flex justify-content-between align-items-center">
                <div><div className="stat-label">{label}</div><div className="stat-value">{value || 0}</div></div>
...
// ========== SHIFT CARD ==========
const ShiftCard = ({ shift, assignedCount, requiredStaff, registered, onRegister, registering, showRegister = true }) => {
    const filled = requiredStaff > 0 ? Math.min(100, (assignedCount / requiredStaff) * 100) : 0;
    const isFull = assignedCount >= requiredStaff;

    let statusColor, statusText, progressVariant;
    if (assignedCount === 0) { statusColor = '#dc3545'; statusText = 'Chưa có ai'; progressVariant = 'danger'; }
    else if (isFull && assignedCount <= requiredStaff) { statusColor = '#28a745'; statusText = 'Đã đủ'; progressVariant = 'success'; }
    else if (assignedCount > requiredStaff) { statusColor = '#17a2b8'; statusText = 'Đã đủ (dư)'; progressVariant = 'info'; }
    else { statusColor = '#ffc107'; statusText = `Thiếu ${requiredStaff - assignedCount}`; progressVariant = 'warning'; }

    return (
        <Card className="shift-card mb-3" style={{ borderLeft: `4px solid ${statusColor}`, borderRadius: '12px', backgroundColor: registered ? '#f0fff4' : 'white' }}>
            <Card.Body className="py-3 px-4">
                <div className="d-flex justify-content-between align-items-center">
                    <div className="flex-grow-1">
                        <div className="d-flex align-items-center gap-2 mb-2">
                            <h6 className="mb-0 fw-bold">{shift.name || 'Ca'}</h6>
                            {shift.shiftAllowance > 0 && <Badge bg="warning" text="dark" className="px-2"><FaDollarSign className="me-1" />+{formatCurrency(shift.shiftAllowance)}</Badge>}
                            {registered && <Badge bg="success" className="px-2"><FaCheck className="me-1" />Đã đăng ký</Badge>}
                        </div>
                        <div className="d-flex align-items-center gap-3 text-muted small mb-2">
                            <span><FiClock className="me-1" size={14} />{formatTime(shift.startTime)} - {formatTime(shift.endTime)}</span>
                            {shift.workingHours && <span><FaHourglassHalf className="me-1" size={14} />{shift.workingHours}h</span>}
                        </div>
                        <div className="d-flex align-items-center gap-2">
                            <div style={{ width: '120px' }}><ProgressBar now={filled} variant={progressVariant} style={{ height: '8px', borderRadius: '4px' }} /></div>
                            <Badge bg={isFull ? 'success' : 'warning'} className="px-2">{assignedCount}/{requiredStaff}</Badge>
                            <small className="text-muted">{statusText}</small>
                        </div>
                    </div>
                    {showRegister && (
                        <Button variant={registered ? 'outline-success' : 'primary'} size="sm" className="px-4 rounded-pill"
                            disabled={registered || isFull || registering} onClick={onRegister} style={{ minWidth: '100px' }}>
                            {registering ? <Spinner size="sm" /> : registered ? 'Đã đăng ký' : isFull ? 'Đã đủ' : 'Đăng ký'}
                        </Button>
                    )}
                </div>
            </Card.Body>
...
const ShiftRegistration = () => {
    ...
    const initUserData = async () => {
        ...
        try {
            ...
            if (!sid && bid && data.id) {
                try { const res = await axiosClient.get(`/staff/branch/${bid}`); const found = res.data.find(s => (s.userId || s.user?.id) === data.id); sid = found?.id || data.id; } catch (e) { sid = data.id; }
            }
            if (!sid) sid = data.id;
            setStaffId(sid); setBranchId(bid);
            setStaffInfo({ id: sid, position: data.position, user: { id: data.id, fullName: data.fullName || data.username }, branch: { id: bid, name: bname } });
    ...
    const loadDayData = useCallback(async (date) => {
        ...
        try {
            ...
            const filtered = allMonthSchedules.filter(s => { const sd = s.workDay || s.work_day; return sd === ds || moment(sd).format('YYYY-MM-DD') === ds; });
    ...
    const handleRegister = async (shiftId) => {
        ...
        try {
            ...
            preloadMonthData(currentMonth); loadDayData(selectedDate);
        } catch (err) { showToast('error', 'Lỗi', err.response?.data?.message || 'Không thể đăng ký'); }
    ...
    const openDetail = (ss) => {
        setSelectedStaffShift(ss); setTodayAttendance(null); setShowDetailModal(true);
        const today = moment().format('YYYY-MM-DD'), sd = moment(ss.workDay).format('YYYY-MM-DD');
        if (today === sd) {
            const sc = allMonthSchedules.find(s => (s.shift?.id || s.shiftId) === (ss.shift?.id || ss.shiftId) && ((s.workDay || s.work_day) === sd || moment(s.workDay || s.work_day).format('YYYY-MM-DD') === sd));
    ...
    const fetchAttendance = async (scheduleId) => {
        ...
        try { const res = await axiosClient.get(`/attendance/today/${staffId}`, { params: { shiftScheduleId: scheduleId } }); setTodayAttendance(res.data); }
    ...
    const handleCheckIn = async () => {
        ...
        setCheckingIn(true);
        try { await axiosClient.post(`/attendance/check-in/${staffId}`, null, { params: { shiftScheduleId: sc.id } }); showToast('success', 'OK', 'Check-in!'); fetchAttendance(sc.id); fetchMonthlyStats(); }
    ...
    const calendarEvents = useMemo(() => {
        return allMonthShifts.map(ss => {
            ...
            return { id: ss.id, title: shift?.name || 'Ca', start: moment(ss.workDay).toDate(), end: moment(ss.workDay).toDate(), resource: ss, allDay: true };
        });
            ...
            <Container fluid>
                        ...
                        <div className="d-flex justify-content-between align-items-center">
                            <div>
                                ...
                                <div className="d-flex align-items-center gap-3 mt-2">
                                    ...
                                    {staffInfo?.position && <Badge bg="primary" className="px-3 py-2 rounded-pill">{staffInfo.position}</Badge>}
                                </div>
                            </div>
                            <Button variant="outline-secondary" size="sm" className="rounded-pill" onClick={() => preloadMonthData(currentMonth)}><FaSyncAlt className="me-1" size={12} />Tải lại</Button>
                        </div>
                    </Card.Body>
                </Card>
                ...
                {monthlyStats && (
                    <Row className="mb-4 g-3">
                        {[{ icon: FaChartBar, label: 'Tổng ngày', val: monthlyStats.totalDays, variant: 'primary' }, { icon: FaCheckCircle, label: 'Đúng giờ', val: monthlyStats.presentDays, variant: 'success' }, { icon: FaExclamationTriangle, label: 'Đi trễ', val: monthlyStats.lateDays, variant: 'warning' }, { icon: FaCalendarAlt, label: 'Nghỉ phép', val: monthlyStats.leaveDays, variant: 'info' }].map((s, i) => (
                            <Col key={i} xs={6} md={3}><StatCard icon={s.icon} label={s.label} value={s.val} variant={s.variant} /></Col>
                        ))}
                    </Row>
                )}
                ...
                <Card className="border-0 shadow-sm" style={{ borderRadius: '16px', overflow: 'hidden' }}>
                    <Card.Header className="bg-white border-0 pt-3 px-4">
                        <Tabs activeKey={viewMode} onSelect={k => setViewMode(k)} className="border-0">
                            ...
                            <Tab eventKey="calendar" title={<span className="px-2"><FaCalendarAlt className="me-2" />Lịch theo tuần</span>} />
                        </Tabs>
                    </Card.Header>
                    <Card.Body className="p-4">
                        {viewMode === 'list' && (
                                ...
                                {loading ? <ShiftSkeleton /> : (
                                    <>
                                        {staffShifts.length > 0 && (
                                            <div className="mb-4">
                                                <h6 className="fw-bold mb-3" style={{ color: '#28a745' }}><FaCheckCircle className="me-2" />Ca đã đăng ký ({staffShifts.length})</h6>
                                                {staffShifts.map(ss => (
                                        ...
                                        {shiftSchedules.length > 0 && (
                                            <div>
                                                <h6 className="fw-bold mb-3" style={{ color: '#4361ee' }}><FiClock className="me-2" />Ca có sẵn ({shiftSchedules.length})</h6>
                                                {shiftSchedules.map(sc => {
                                                    ...
                                                    return <ShiftCard key={sc.id} shift={shift} assignedCount={assignedCount} requiredStaff={sc.requiredStaff || 0} registered={registered} onRegister={() => handleRegister(shift.id)} registering={registeringShiftId === shift.id} />;
                                                })}
                                            </div>
                                        )}
                                        {staffShifts.length === 0 && shiftSchedules.length === 0 && (
                                            <div className="text-center py-5"><FaCalendarAlt style={{ fontSize: '4rem', opacity: 0.3 }} /><p className="text-muted mt-3">Không có ca làm nào</p></div>
                                        )}
                                    </>
                                )}
                            </>
                ...
                <Modal show={showDetailModal} onHide={() => setShowDetailModal(false)} centered>
                    ...
                    <Modal.Body className="p-4">
                        {selectedStaffShift && (
                            <>
                                <Row className="mb-4">
                                    <Col xs={6}><div className="text-center p-3 rounded-3" style={{ background: '#f8f9fa' }}><small className="text-muted d-block mb-1"><FaCalendarAlt className="me-1" />Ngày làm</small><strong className="d-block" style={{ fontSize: '1.2rem' }}>{moment(selectedStaffShift.workDay).format('DD/MM/YYYY')}</strong>{isToday(selectedStaffShift.workDay) && <Badge bg="warning" className="mt-1 rounded-pill">Hôm nay</Badge>}</div></Col>
                                    <Col xs={6}><div className="text-center p-3 rounded-3" style={{ background: '#f8f9fa' }}><small className="text-muted d-block mb-1"><FiClock className="me-1" />Ca làm</small><strong className="d-block" style={{ fontSize: '1.2rem' }}>{selectedStaffShift.shift?.name}</strong><small className="text-muted">{formatTime(selectedStaffShift.shift?.startTime)} - {formatTime(selectedStaffShift.shift?.endTime)}</small></div></Col>
                                </Row>
                                {isToday(selectedStaffShift.workDay) && (
                                    <div className="p-3 rounded-3 mb-3" style={{ background: '#f0f4ff', border: '2px solid #4361ee' }}>
                                        ...
                                        {attendanceLoading ? <div className="text-center"><Spinner size="sm" /></div> : todayAttendance ? (
                                            <div>
                                        ...
                                        <div className="d-flex gap-2 mt-3">
                                            <Button variant="success" className="flex-fill rounded-pill" disabled={todayAttendance?.checkIn || checkingIn} onClick={handleCheckIn}>{checkingIn ? <Spinner size="sm" /> : todayAttendance?.checkIn ? <><FaCheck className="me-1" />Đã check-in</> : <><FiClock className="me-1" />CHECK-IN</>}</Button>
                                            <Button variant="danger" className="flex-fill rounded-pill" disabled={!todayAttendance?.checkIn || todayAttendance?.checkOut || checkingOut} onClick={handleCheckOut}>{checkingOut ? <Spinner size="sm" /> : todayAttendance?.checkOut ? <><FaCheck className="me-1" />Đã check-out</> : <><FaTimes className="me-1" />CHECK-OUT</>}</Button>
                                        </div>
                                    </div>
                                )}
                            </>
                    ...
                    <Modal.Footer>
                        <Button variant="outline-danger" className="rounded-pill" onClick={() => handleCancel(selectedStaffShift?.id)}><FaTrashAlt className="me-1" />Hủy đăng ký</Button>
