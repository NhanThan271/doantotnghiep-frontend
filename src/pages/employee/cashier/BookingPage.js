...
const BookingPage = () => {
    ...
    // ==================== SOCKET.IO SETUP ====================
    useEffect(() => {
        // Đăng ký role cho nhân viên thu ngân
        ...
        socket.on("connect", () => {
            ...
            // Đăng ký lại role khi reconnect
            if (userData.id && branchId) {
                socket.emit("register-role", {
                    role: "cashier",
                    userId: userData.id,
        ...
        // Lắng nghe thông báo đặt bàn từ staff
        ...
        // Lắng nghe cập nhật bàn từ socket
        ...
        // Lắng nghe đặt bàn mới từ khách hàng
        ...
        // 👇 THÊM: Lắng nghe cập nhật reservation từ user khác
    ...
    const confirmReservation = async (reservationId) => {
        try {
            ...
            if (response.ok || response.status === 500) {
                ...
                // 👇 THÊM: Emit để báo scheduler biết
        ...
        <div className={styles.bookingPage}>
            ...
            <div className={styles.tabNavigation}>
                ...
                >
                    <Table size={18} />
                    Bàn ăn
                    <span className={styles.tabCount}>{tables.length}</span>
                </button>
            ...
            {activeTab === "tables" && areas.length > 0 && (
                <div className={styles.areaFilter}>
                    ...
                    >
                        <MapPin size={16} />
                        <span>Khu vực: {selectedArea}</span>
                        ...
                        <div className={styles.areaList}>
                            {areas.map(area => (
                                <button
                                    ...
                                    className={`${styles.areaButton} ${selectedArea === area ? styles.activeArea : ""}`}
                                >
                        ...
                        <div className={styles.grid}>
                            {tables.map((table) => {
                                ...
                                return (
                                    <div
                                        key={table.id}
                                        className={`${styles.card} ${isOccupied ? styles.cardOccupied :
                                    ...
                                    >
                                        ...
                                        <div className={styles.cardContent}>
                                            <div className={styles.cardHeader}>
                                                ...
                                                {table.hasUpcomingReservation && (
                                                    <span className={styles.upcomingBadge} title="Có đặt bàn sắp tới">
                                                        <Clock size={10} />
                                                    </span>
                                                )}
                                            </div>
                                            ...
                                            {table.hasUpcomingReservation && table.customerName && (
                                                <div className={styles.upcomingInfo}>
                                                    <AlertTriangle size={12} />
                                                    <div>
                                                        <p className={styles.upcomingName}>
                                                            {table.customerName}
                                                        </p>
                                                        {table.checkInTime && (
                                                            <p className={styles.upcomingTime}>
                                                                {formatDateTime(table.checkInTime)}
                                                            </p>
                                                        )}
                                                    </div>
                                                </div>
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div className={styles.cardContent}>
                                            ...
                                            {room.hasUpcomingReservation && room.customerName && (
                                                <div className={styles.upcomingInfo}>
                                                    <AlertTriangle size={12} />
                                                    <div>
                                                        <p className={styles.upcomingName}>
                    ...
                    <div className={styles.reservationsSection}>
                        ...
                        {reservations.length === 0 ? (
                            <div className={styles.emptyState}>
                                ...
                                <p>Tất cả đặt bàn đã được xử lý</p>
                            </div>
                        ) : (
                                    ...
                                    <div key={res.id} className={styles.reservationCard}>
                                        ...
                                        <div className={styles.reservationDetails}>
                                            <div className={styles.detailItem}>
                                                <Phone size={14} />
                                                <span>{res.phone || "Không có"}</span>
                                            </div>
                                            <div className={styles.detailItem}>
                                                <Calendar size={14} />
                                                <span>{formatDateTime(res.checkInTime)}</span>
                                            </div>
                                            <div className={styles.detailItem}>
                                                {res.tableNumber ? (
                                                    <>
                                                    ...
                                                    <>
                                                        <Home size={14} />
                                                        <span>Phòng {res.roomNumber}</span>
                                                    </>
                                                ) : (
                                                    <>
                                                        <MapPin size={14} />
                                            ...
                                            {res.remainingAmount > 0 && (
                                                <div className={styles.detailItem}>
                                                    <span className={styles.amount}>
                                                        Còn lại: {formatCurrency(res.remainingAmount)}
                                                    </span>
                                                </div>
                                            )}
                                        </div>
                        ...
                        <div className={styles.modalBody}>
                            <div className={styles.infoBox}>
                                <div className={styles.infoItem}>
                                    <span>
                                        {selectedEntity.type === "table" ? "🍽️" : "🏠"}
                                        <strong>
                                            {selectedEntity.type === "table" ? "Bàn" : "Phòng"} {selectedEntity.number}
                                        </strong>
                                    </span>
                                    {selectedEntity.capacity && (
                                        <span>
                                            <Users size={14} />
                                            {selectedEntity.capacity} người
                                        </span>
                                    )}
                                </div>
                                {selectedEntity.area && (
                            ...
                            <form onSubmit={(e) => { e.preventDefault(); handleSubmitBooking(); }}>
                                ...
                                <div className={styles.notificationInfo}>
                                    <Bell size={14} />
                                    <span>
                                        Đặt bàn sẽ ở trạng thái chờ xác nhận. Hệ thống sẽ gửi thông báo cho phục vụ và bếp.
                                    </span>
                                </div>
                            </form>
                        </div>
