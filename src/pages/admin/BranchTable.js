...
export default function TableManagement() {
    ...
    const confirmDelete = async (id) => {
        const token = localStorage.getItem('token');
        const response = await fetch(`${API_BASE_URL}/api/tables/${deleteConfirm}`, {
            method: 'DELETE',
            headers: { Authorization: `Bearer ${token}` }
        })
            .then(res => {
                if (!res.ok) throw new Error('Xóa thất bại');
                showToast('success', 'Xóa thành công!', 'Bàn đã được xóa.');
                setDeleteConfirm(null);
                fetchTables();
            })
            .catch(err => {
                console.error(err);
                showToast('error', 'Xóa thất bại', 'Không thể xóa bàn. Vui lòng thử lại!');
                setDeleteConfirm(null);
            });
    };
                ...
                <div style={{
                    ...
                    <div style={{
                        ...
                        <p style={{ color: '#B8B8B8', fontSize: '14px', marginBottom: '24px' }}>
                            Bạn có chắc muốn xóa bàn này? Hành động này không thể hoàn tác!
                        </p>
